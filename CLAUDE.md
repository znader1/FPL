...
`build_chip_plan()` answers "which chip, which GW": per-chip EV per candidate GW (TC = captain's extra ×1; BB = bench-4 sum; FH = budget-constrained rebuilt XI vs own, gated on ≥`CHIP_PLAN_FH_MIN_BLANKING` blanking squad players OR ≥`CHIP_PLAN_FH_MIN_TOUGH` squad players facing difficulty ≥`CHIP_PLAN_FH_TOUGH_DIFFICULTY` (tough-pileup gate) OR, since 2026-09-18, a blended squad-stress score ≥`CHIP_PLAN_FH_MIN_STRESS` — per player `max(blank, 1 − play_prob, tough_weight)` summed over the 15, with `tough_weight` ramping from 0 at `CHIP_PLAN_FH_STRESS_TOUGH_FROM` (3.3, the ticker's medium ceiling) to 1 at difficulty 5, so three injured players plus a hard away week surfaces a FH row even when neither hard count is met; `play_prob` = availability (FPL `status`/`chance_of_playing_next_round`, `chip_advisor.play_prob_from_availability`) × P(start) from the projection frame's `recent_gw_avg_starts` shrunk toward 1.0 by `CHIP_PLAN_FH_BENCH_PRIOR_GWS` pseudo-GWs (`start_prob_from_recent_starts`: a fit player benched 3 of 3 → 0.25, so a manager's selection call counts even without an FPL flag), both set in `api/chat._build_context_for_entry` (absent → everyone fit and starting); an injury-driven opener adds a `Risk:` line to check free transfers first; a HELD FH row is never a bare "no window" — `fh_best_stress_row` reports the most-stressed candidate GW as `outlook[].stress` (`total`/`bar`/`gw` + the counts and names behind it) and `describe_fh_stress` writes the one shared clause used by both the gate's reasoning and the hold row/guidance, suppressed only when the reading is 0; WC = budget/team-cap-constrained optimizer squad summed over the horizon, net of the transfer plan), expiry-aware chip windows (phase 1/2 via `chip_windows`, FPL names normalized there), min-EV thresholds with an expiry urgency ramp (`effective_min_ev`), a structural zone beyond the model horizon (announced DGW/BGW → `provisional` recs), and a next-GW `nudge`. Dream-squad sides clamp xpts position-aware via `CHIP_PLAN_XPTS_CLAMP_BY_POS` (flat `CHIP_PLAN_XPTS_CLAMP` fallback when a market has no `pos` column). Optional strategy signals sharpen the picture further: a rec targeting a post-international-break GW gets a confidence haircut (`CHIP_PLAN_BREAK_CONFIDENCE_MULT`) and the next-GW nudge sets `wait_for_team_news`; WC/TC recs near an easier fixture-difficulty swing get a reasons entry (TC only when the swing is the recommended captain's own team); and TC carries a Poisson-derived `haul_prob` from the captain's per-90 xGI and upcoming fixture difficulty. All tunables `CHIP_PLAN_*`; thresholds are live-tuned, backtest pending.
